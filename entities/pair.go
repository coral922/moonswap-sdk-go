...
func getCreate2Address(addressA, addressB common.Address) common.Address {
	...
	calAddr := crypto.CreateAddress2(constants.FactoryAddress, salt, constants.InitCodeHash)
	return wrapperCfxAddress(calAddr)
}

func wrapperCfxAddress(address common.Address) common.Address {
	bytes := address.Bytes()
...
func GetPairAddress(token1Addr, token2Addr common.Address) common.Address {
	return _PairAddressCache.GetAddress(token1Addr, token2Addr)
}
...
func NewPair(tokenAmountA, tokenAmountB *TokenAmount) (*Pair, error) {
	...
	pair.LiquidityToken, err = NewToken(tokenAmountA.Token.ChainID, pair.GetAddress(),
		constants.Decimals18, constants.MoonSwapSymbol, constants.MoonSwapName)
	return pair, err
}
